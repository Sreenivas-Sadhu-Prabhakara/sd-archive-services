...
| **Asset Type** | Archive Item |
| **Control Record** | Archive Item Maintenance Agreement |
...
| POST | `/v1/archive-item-maintenance-agreement/initiate` | Initiate |
| GET | `/v1/archive-item-maintenance-agreement` | Retrieve (list) |
| GET | `/v1/archive-item-maintenance-agreement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/archive-item-maintenance-agreement/{crId}/update` | Update |
| PUT | `/v1/archive-item-maintenance-agreement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/archive-item-maintenance-agreement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-archive-services:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-archive-services ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-archive-services` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
